...
namespace Crosstales.RTVoice.EditorExt
{
    ...
    public class RTVoiceMenu
    {
        ...
        {
            PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath("Assets" + Constants.PREFAB_PATH + "SpeechText.prefab", typeof(GameObject)));
        }

        [MenuItem("Tools/RTVoice/Add/Sequencer", false, EditorHelper.MENU_ID + 50)]
        private static void AddSequencer()
        {
            PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath("Assets" + Constants.PREFAB_PATH + "Sequencer.prefab", typeof(GameObject)));
        }

        [MenuItem("Tools/RTVoice/Add/TextFileSpeaker", false, EditorHelper.MENU_ID + 60)]
        private static void AddTextFileSpeaker()
        {
            PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath("Assets" + Constants.PREFAB_PATH + "TextFileSpeaker.prefab", typeof(GameObject)));
        }

        [MenuItem("Tools/RTVoice/Add/Loudspeaker", false, EditorHelper.MENU_ID + 80)]
        private static void AddLoudspeaker()
        {
            PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath("Assets" + Constants.PREFAB_PATH + "Loudspeaker.prefab", typeof(GameObject)));
        }
