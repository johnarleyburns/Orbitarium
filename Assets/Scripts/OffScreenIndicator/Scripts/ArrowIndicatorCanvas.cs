...
namespace Greyman{
	public class ArrowIndicatorCanvas : ArrowIndicator {
		...
		public override void UpdateEffects(){
            /* JAB NEW */
            if (indicator.hasOnScreenText && indicator.showOnScreen)
            {
                if (text == null)
                {
                    text = new GameObject();
                    text.name = "Indicator Label";
                    text.transform.SetParent(arrow.transform);
                    text.AddComponent<Text>();
                    text.GetComponent<Text>().text = indicator.onScreenTextString;
                    text.GetComponent<Text>().font = indicator.onScreenFont;
                    text.GetComponent<Text>().fontStyle = indicator.onScreenFontStyle;
                    text.GetComponent<Text>().fontSize = indicator.onScreenFontSize;
                    text.GetComponent<Text>().color = indicator.onScreenFontColor;
                    text.GetComponent<Text>().alignment = TextAnchor.UpperCenter;
                    text.GetComponent<Text>().rectTransform.sizeDelta = new Vector2(indicator.onScreenTextWidth, indicator.onScreenTextHeight);
                    text.GetComponent<Text>().transform.localPosition = new Vector3(0, indicator.onScreenTextPosY, 0);
                    text.GetComponent<Text>().transform.localRotation = Quaternion.identity;
                }
                text.GetComponent<Text>().text = indicator.onScreenTextString; // update for changes
            }
            /* JAB END */
