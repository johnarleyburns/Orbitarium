...
public class Console : UIBehaviour, IUpdateSelectedHandler, IPointerDownHandler
{
    ...
    [SerializeField]
    protected bool _singleton = false;
    [SerializeField]
            ...
            if (!_inputKeyboard.active && _inputKeyboard != null)
            {
                if (GetInputLine() != string.Empty || GetInputLine() != "")
                {
                    if (_useEscapeChar)
                    {
                        if (!_isPasswordLine)
                        {
                            _consoleText.text = _consoleText.text.Replace(GetInputLine(), _escapeChar + _text);
                        ...
                        else
                        {
                            if (usePasswordChar)
                            {
                                _consoleText.text = _consoleText.text.Replace(GetInputLine(), _escapeChar + new string(_passwordChar, _text.Length));
                                _index = _text.Length + 1;
                            }
                        }
                    }
                    else
                    {
                        if (!_isPasswordLine)
                        {
                            _consoleText.text = _consoleText.text.Replace(GetInputLine(), _text);
                            _index = _text.Length;
                        }
                        else
                        {
                            if (usePasswordChar)
                            {
                                _consoleText.text = _consoleText.text.Replace(GetInputLine(), new string(_passwordChar, _text.Length));
                                _index = _text.Length;
                            }
                        }
                    }
