...
//		outOfBoundsSquared = gravityEngine.outOfBounds * gravityEngine.outOfBounds; 
		...
		if (burstCount == gravityParticles.maxParticles) {
			oneTimeBurst = true;
			burstDone = false;
		}
		...
		if (debugLogs) {
			Debug.Log(this.name + " start: oneTimeBurst=" + oneTimeBurst + " burstCount=" +
						 burstCount + " pc=" + gravityParticles.maxParticles);
		}
	...
	private void InitParticleData() {
		...
		if (gravityParticles.simulationSpace == ParticleSystemSimulationSpace.Local) {
			Debug.LogWarning("Particle simulation space must be set to World. Fixing.");
			gravityParticles.simulationSpace = ParticleSystemSimulationSpace.World;
		}
		if (gravityParticles.duration > 9999) {
			Debug.LogWarning("Duration > 9999 has resulted in no particles visible in the past.");
		}
		// create array to hold particles
		particles = new ParticleSystem.Particle[gravityParticles.maxParticles]; 
		// get particles from the system (this fills in particles[])
		gravityParticles.GetParticles(particles);
		int maxParticles = gravityParticles.maxParticles;
		#pragma warning disable 162		// disable unreachable code warning
		if (debugLogs) {
...
//							if (allowRemoval && rsq_wrt0 > outOfBoundsSquared) {
//								#pragma warning disable 162		// disable unreachable code warning
//								if (debugLogs) 
//									Debug.Log (string.Format ("Ejected particle {0}  p={1} {2} rsq={3}", j, r[j,0], r[j,1], rsq_wrt0));
//								#pragma warning restore 162
//								ejectCount++;
//								inactive[j] = true;
//								r[j,0] = OUT_OF_VIEW;
