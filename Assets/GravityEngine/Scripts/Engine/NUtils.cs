...
public class NUtils {
    ...
    {
        NBodyDimensions d = GetNBodyDimensions(body);
        if (d != null)
        {
            return d.NBody;
        }
    ...
    private static NBodyDimensions GetNBodyDimensions(GameObject body)
    {
        ...
        {
            body = body.transform.parent.gameObject;
            d = body.GetComponent<NBodyDimensions>();
        }
        return d;
    }

    public static float GetNBodyToModelScale(GameObject body)
    {
        NBodyDimensions d = GetNBodyDimensions(body);
        if (d != null)
        {
            return d.NBodyToModelScaleFactor;
        }
        else
        {
            return 1;
