...
//	private static double RSQ_EJECTION = GravityEngine.instance.outOfBounds * GravityEngine.instance.outOfBounds;
	...
	/// <summary>
	/// Evolve the position of the active masses based on the gravitational field by the
	...
	public double Evolve(double evolveFor, int numMasses, ref double[] m, ref double[,] r_m, ref double[] size2, ref byte[] massive_info) {
		if (numMasses == 0 && numBodies == 0) {
			return evolveFor;
		}
...
//							if (rsq_wrt0 > RSQ_EJECTION) {
//								ejectCount++;
//								#pragma warning disable 162		// disable unreachable code warning
//								if (GravityEngine.DEBUG) {
//									Debug.Log("Ejected " + bodies[j]);
//								}
//								#pragma warning restore 162		
